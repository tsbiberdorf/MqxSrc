...
The script analyzes the generated xMAP file produced by CodeWarrior,
IAR Embedded Workbench or ARM build tools and creates codesize report in 
an HTML format.

The script PERL source code is located in the current folder, the script 
executable is located in /tools directory. 

The BSP and PSP libraries as well as the application code needs to be
built in a special way in different configurations and different include
search-paths to the customized user_config.h files. See the "mk" folder 
for custom makefiles used for this purpose. Note that the makefiles depend
on the master Makefiles of the MQX in the top-level /mk folder. 

The reports created with this script for different build tools are available 
in the "report" folder. The MQX kernel configurations used for these results
